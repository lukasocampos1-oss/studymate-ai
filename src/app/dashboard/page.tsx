      ...
      while (true) {
        ...
        for (const line of lines) {
          ...
          try {
            ...
            const questions2 = parseQuiz(fullText)
            if (questions2.length > 0) setQuestions(questions2)
          } catch {}
        }
      }
              ...
              history.map(item => (
                <div key={item.id} className="bg-white rounded-xl shadow p-6 flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => loadHistoryQuiz(item)} className="bg-blue-50 text-blue-600 px-4 py-2 rounded-lg hover:bg-blue-100 text-sm font-medium">
                      🔄 Retake
                    </button>
                    <button onClick={() => deleteHistory(item.id)} className="bg-red-50 text-red-500 px-4 py-2 rounded-lg hover:bg-red-100 text-sm font-medium">
                      🗑️
                    </button>
                  </div>
                </div>
          ...
          <>
            <div className="bg-white rounded-xl shadow p-6 mb-6">
              ...
              {inputMode === 'text' ? (
                ...
                >
                  ...
                  <p className="text-gray-600 font-medium">{fileName || 'Click to upload .docx, .pdf or .txt file'}</p>
            ...
            {quizDone && (
              <div className={`rounded-xl border-2 p-6 mb-6 text-center ${scoreBg}`}>
                ...
                <button onClick={resetQuiz} className="bg-blue-600 text-white px-6 py-2 rounded-lg hover:bg-blue-700 font-medium">
                  🔄 Retake Quiz
                </button>
              </div>
            )}
            ...
            {questions.length > 0 && (
              <div className="space-y-4">
                <div className="flex justify-between items-center">
                  ...
                  <div className="flex gap-2">
                    <button onClick={revealAll} className="text-sm bg-green-50 text-green-600 px-3 py-1 rounded-lg hover:bg-green-100">👁️ Reveal All</button>
                    <button onClick={hideAll} className="text-sm bg-gray-100 text-gray-600 px-3 py-1 rounded-lg hover:bg-gray-200">🙈 Hide All</button>
                  </div>
                </div>
                  ...
                  <div key={i} className={`bg-white rounded-xl shadow p-6 border-l-4 ${q.correct === true ? 'border-green-400' : q.correct === false ? 'border-red-400' : 'border-transparent'}`}>
                    ...
                    {q.correct === null && (
                      <div className="mt-3 flex gap-2">
                        <button onClick={() => markAnswer(i, true)} className="flex-1 bg-green-100 text-green-700 py-2 rounded-lg hover:bg-green-200 font-medium text-sm">
                          ✅ Got it right
                        </button>
                        <button onClick={() => markAnswer(i, false)} className="flex-1 bg-red-100 text-red-700 py-2 rounded-lg hover:bg-red-200 font-medium text-sm">
                          ❌ Got it wrong
                        </button>
                      </div>
                    )}
